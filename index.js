...
function signIn() {
  ...
  //   createButton.value = "Create User";
  ...
  //     submitButton.value = "Create User and Log In";
...
//   //fetch create to backend, then fetchUser(event, username) after creation.
    ...
    .then((usersArray) => {
      usersArray["data"].forEach((arrayItem) => {
        if (username === arrayItem["attributes"]["username"]) {
          ...
          localStorage.setItem("user", arrayItem["id"]);
...
function displayUserHighScores() {
  ...
  fetch(`http://localhost:3000/users/${header.id}`)
    ...
    .then((userData) => {
      ...
      let sortedPoints = pointsArray.sort(function (a, b) {
        return b - a;
      });
...
function fetchSong() {
  ...
  fetch("http://localhost:3000/songs/answer_songs")
    ...
    .then((songData) => {
      ...
      const songChoice =
        sortedSongs[Math.floor(Math.random() * sortedSongs.length)];
...
function isUserHiScore() {
  ...
  let isCurrentAHiScore = userHiScoresArray[userHiScoresArray.length - 1];

  //   let userScoreResult = isCurrentAHiScore < score;
  if (score > isCurrentAHiScore) {
    displayUserHighScores();
  }
